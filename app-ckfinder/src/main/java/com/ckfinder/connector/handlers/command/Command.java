...
public abstract class Command {
	...
	protected boolean checkCsrfToken(final HttpServletRequest request, String csrfTokenValue) {
		...
		String user = UtilsCommon.getUserNameLogin();
		Authentication authentication = UtilsCommon.getAuthentication();

		if (StringUtils.isBlank(user)) return false;
		if (ConstantAuthor.contain(ConstantAuthor.Ckfinder.author, authentication)) {
			return true;
		}
